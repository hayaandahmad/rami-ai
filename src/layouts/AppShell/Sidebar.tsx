...
export function Sidebar({ collapsed = false, onNavigate, id = "sidebar-navigation" }: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      <nav className={`flex-1 overflow-hidden ${collapsed ? "px-2 py-4" : "px-3 py-4"}`}>
              ...
              <li key={item.label}>
                <Link
                  ...
                  className={`relative flex min-h-11 items-center rounded-control text-small transition-hover focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-sidebar-accent-bar ${
                    ...
                    isActive
                      ? "bg-sidebar-active font-medium text-sidebar-text-primary shadow-[inset_0_0_0_1px_rgba(255,255,255,0.08)]"
                ...
                >
                  {isActive ? (
                    <span
                      aria-hidden="true"
                      className={`absolute top-1/2 h-7 w-1 -translate-y-1/2 rounded-r-full bg-sidebar-accent-bar ${
                  ...
                  <Icon
                    aria-hidden="true"
                    className={`h-5 w-5 shrink-0 ${isActive ? "text-sidebar-text-primary" : ""}`}
                    strokeWidth={1.75}
                  />
